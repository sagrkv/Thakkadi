 ...
 * 1. Finds applicable legal options based on case details
 * 2. Calculates limitation periods with exclusions
 * 3. Returns structured, explainable results
 *
 * DESIGN PRINCIPLE: Deterministic, rule-based, fully auditable
 * NO AI/ML used in calculations - only in explanations (separate layer)
 */
...
export function calculateLimitation(input: CaseInput): CalculationResult {
  ...
  // Step 3: Calculate limitation for each applicable option
  const options: LegalOption[] = applicableRules.map((rule) => {
    return calculateOption(input, rule, masterAuditLog);
  });

  // Sort by days remaining (most urgent first), then by confidence
  options.sort((a, b) => {
    if (a.isExpired !== b.isExpired) {
      return a.isExpired ? 1 : -1; // Non-expired first
    }
    if (a.daysRemaining !== b.daysRemaining) {
      return a.daysRemaining - b.daysRemaining; // Fewer days remaining first
    }
...
/**
 * Calculate a single legal option
 */
...
/**
 * Format limitation period for display
 */
...
export function getResultsSummary(result: CalculationResult): {
  ...
  urgentOptions: number; // Less than 7 days
